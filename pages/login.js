...
const Login = () => {
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      didToken = magic.auth
        ...
        .then((decentralisedId) => console.log(decentralisedId))
        .catch((err) => {
  ...
  //     email,
  //     showUI: false,
  //     redirectURI: `${window.location.origin}/login/magic${isLoggedIn ? '?magic_verification_only=1' : ''}`,
  //   })
  //     .on('email-sent', () => {
  //       setIsEmailSent(true);
  //       setLoadingState(IDialogStatus.LOADED);
  //     })
  //     .on('email-not-deliverable', () => {
  //       handleError('The email address you entered is undeliverable.');
  //     })
  //     .then(decentralisedId => console.log(decentralisedId!))
  //     .catch(err => {
  //       if (err.code) {
